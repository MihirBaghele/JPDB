...
export const RowCalculationDialog = ({
  ...
  const handleGetAllAPiData = async (): Promise<void> => {
    try {
      ...
      console.log('Region from props:', region);

      const forecastId = activeForecastItems?.traditional_forecast_id || 61;
      console.log('Using forecast ID:', forecastId);

      if (cardType === 'Originator Volume') {
        const getOriVolRows = await getRowOriginatorVolume(forecastId);
        console.log('✅ Originator Volume API response:', getOriVolRows);

        const newSetRows = [...rows];
        const currentThirdRow = newSetRows[2];
        const apiData = Array.isArray(getOriVolRows) ? getOriVolRows : [];

        const countryToMatch = currentThirdRow[0]?.value;
        console.log('Looking for country:', countryToMatch);
        console.log('Available countries in API:', apiData.map((item: any) => item.country));

        const filteredApiValue = apiData.filter(
          (rowEle: any) => rowEle.country === countryToMatch
        );

        console.log('Filtered data:', filteredApiValue);

        if (filteredApiValue && filteredApiValue.length > 0) {
          const apiRecord = filteredApiValue[0];
          
          // Map rowOrgvolfactor to row 2
          apiRecord.rowOrgvolfactor?.forEach((e: any) => {
            newSetRows[2].forEach((re: any) => {
              if (e.year == re.key) {
                console.log(`Mapping volume factor: year ${e.year} = ${e.value}`);
                re.value = e.value;
              }
            });
          });
          ...
          apiRecord.rowOriginatorMarketSize?.forEach((e: any) => {
            newSetRows[3].forEach((re: any) => {
              if (e.year == re.key) {
                console.log(`Mapping market size: year ${e.year} = ${e.value}`);
                re.value = e.value;
              }
            });
          });
          ...
          apiRecord.rowOriginatorMarketSizeAfter?.forEach((e: any) => {
            newSetRows[4].forEach((re: any) => {
              if (e.year == re.key) {
                console.log(`Mapping market size after: year ${e.year} = ${e.value}`);
                re.value = e.value;
              }
            });
          });
          ...
          console.warn('⚠️ No matching data found for country:', countryToMatch);
        }
      } else if (cardType === 'Originator Price') {
        const getOriPriceRows = await getRowOriginatorPrice(forecastId);
        console.log('✅ Originator Price API response:', getOriPriceRows);

        const newSetRows = [...rows];
        const currentFactorRow = newSetRows[1];
        const apiData = Array.isArray(getOriPriceRows) ? getOriPriceRows : [];

        const countryToMatch = currentFactorRow[0]?.value;
        console.log('Looking for country:', countryToMatch);
        console.log('Available countries in API:', apiData.map((item: any) => item.country));
        console.log('Current row structure - row[1][0]:', currentFactorRow[0]);

        const filteredApiValue = apiData.filter(
          (rowEle: any) => {
            const matches = rowEle.country === countryToMatch;
            console.log(`Comparing API country "${rowEle.country}" with "${countryToMatch}": ${matches}`);
            return matches;
          }
        );
        ...
        if (filteredApiValue && filteredApiValue.length > 0) {
          const apiRecord = filteredApiValue[0];
          console.log('Using API record:', apiRecord);
          
          // Map rowOrgpricefactor to row 1
          apiRecord.rowOrgpricefactor?.forEach((e: any) => {
            newSetRows[1].forEach((re: any) => {
              if (e.year == re.key) {
                console.log(`✅ Mapping price factor: year ${e.year} = ${e.value}`);
                re.value = e.value ?? '';
              }
            });
          });
          ...
          apiRecord.rowOriginatorExfactorPrice?.forEach((e: any) => {
            newSetRows[2].forEach((re: any) => {
              if (e.year == re.key) {
                console.log(`✅ Mapping ex-factory price: year ${e.year} = ${e.value}`);
                re.value = e.value ?? '';
              }
            });
          });
          ...
          console.warn('⚠️ No matching data found for country:', countryToMatch);
          console.warn('Available data:', apiData);
        }
      } else if (cardType === 'Sandoz Price') {
        const getSandozPricRows = await getRowSandozPrice(forecastId);
        console.log('✅ Sandoz Price API response:', getSandozPricRows);

        const newSetRows = [...rows];
        const currentFactorRow = newSetRows[1];
        const apiData = Array.isArray(getSandozPricRows) ? getSandozPricRows : [];

        const programToMatch = currentFactorRow[1]?.value;
        const countryToMatch = currentFactorRow[0]?.value;
        console.log('Looking for program:', programToMatch, 'and country:', countryToMatch);

        const filteredApiValue = apiData.filter(
          (rowEle: any) =>
            rowEle.programName === programToMatch && rowEle.country === countryToMatch
        );
        ...
        if (filteredApiValue && filteredApiValue.length > 0) {
          ...
          apiRecord.rowSandozpriFac?.forEach((e: any) => {
            newSetRows[1].forEach((re: any) => {
              if (e.year == re.key) {
                console.log(`Mapping Sandoz price factor: year ${e.year} = ${e.value}`);
                re.value = e.value;
              }
            });
          });
          
          apiRecord.rowSandozPrice?.forEach((e: any) => {
            newSetRows[2].forEach((re: any) => {
              if (e.year == re.key) {
                console.log(`Mapping Sandoz price: year ${e.year} = ${e.value}`);
                re.value = e.value;
              }
            });
          });

          console.log('✅ Updated rows for Sandoz Price:', newSetRows);
          setRows(newSetRows);
        } else {
          console.warn('⚠️ No matching data found');
        }
      } else if (cardType === 'Sandoz Volume') {
        const getSandozVolRows = await getRowSandozVolume(forecastId);
        console.log('✅ Sandoz Volume API response:', getSandozVolRows);

        const newSetRows = [...rows];
        const currentFactorRow = newSetRows[1];
        const apiData = Array.isArray(getSandozVolRows) ? getSandozVolRows : [];
        ...
        if (filteredApiValue && filteredApiValue.length > 0) {
          ...
          apiRecord.rowSandozvolFac?.forEach((e: any) => {
            newSetRows[1].forEach((re: any) => {
              if (e.year == re.key) {
                console.log(`Mapping Sandoz volume factor: year ${e.year} = ${e.value}`);
                re.value = e.value ?? '';
              }
            });
          });
          
          apiRecord.rowSandozVolume?.forEach((e: any) => {
            newSetRows[2].forEach((re: any) => {
              if (e.year == re.key) {
                console.log(`Mapping Sandoz volume: year ${e.year} = ${e.value}`);
                re.value = e.value;
              }
            });
          });

          console.log('✅ Updated rows for Sandoz Volume:', newSetRows);
          setRows(newSetRows);
        } else {
          console.warn('⚠️ No matching data found');
        }
  ...
  useEffect(() => {
    // Only fetch if rows have been initialized and dialog is open
    if (rows.length > 0 && isOpen) {
      console.log('useEffect triggered - fetching API data');
      handleGetAllAPiData();
    }
  }, [isOpen]); // Trigger when dialog opens
